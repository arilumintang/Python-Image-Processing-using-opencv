...
grayscale = cv2.imread("grayscale.png", 0)
grayscale = 255 - grayscale
ground_truth = cv2.imread("groundtruth.png", 0)
plt.subplot(1, 3, 1)
...
def get_validation_metrics(groundtruth_list, predicted_list):
    _assert_valid_lists(groundtruth_list, predicted_list)
    validation_metrics = {}
    validation_metrics["accuracy"] = get_accuracy(
        groundtruth_list, predicted_list)
    validation_metrics["f1_score"] = get_f1_score(
        groundtruth_list, predicted_list)
...
validation_metrics = get_validation_metrics(groundtruth_list,
                                            predicted_list)
...
validation_mask = get_confusion_matrix_overlaid_mask(
    255 - grayscale, ground_truth, predicted, alpha, confusion_matrix_colors)
print("Cyan - TP")
print("Magenta - FP")
print("Yellow - FN")
print("Black - TN")
...
def test_get_validation_metrics():
    ...
    nose.tools.assert_almost_equal(
        validation_metrics["f1_score"], 0.75, places=3)
...
def test_get_confusion_matrix_intersection_mats():
    confusion_matrix_mats = get_confusion_matrix_intersection_mats(
        ORIGINAL, PREDICTED)
...
def test_get_confusion_matrix_overlaid_mask():
    ...
    expected = [0, 128, 128, 0, 0, 0, 0, 128, 128, 0, 0, 0,
                128, 128, 0, 0, 128, 128, 128, 0, 128, 0, 0, 0, 0, 0, 0]
    np.testing.assert_array_equal(
        confusion_matrix_mask.flatten().tolist(), expected)
